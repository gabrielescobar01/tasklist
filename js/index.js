...
function addTaskList() {
  ...
  document
    .querySelectorAll(".delete-table")
    .forEach((button) => button.addEventListener("click", removeTaskList));
  document
    .querySelectorAll(".add-row")
    .forEach((button) => button.addEventListener("click", addRowToList));
  document
    .querySelectorAll(".delete-row")
    .forEach((button) => button.addEventListener("click", deleteRowFromList));
...
function createTaskList() {
  ...
  table_body_container.id =
    "table-body-" + document.getElementsByClassName("task-list").length;
  ...
  table_header_container.innerHTML =
    table_header_text.outerHTML + table_delete_button.outerHTML;
  table.innerHTML =
    table_header_container.outerHTML + table_body_container.outerHTML;
...
function regenerateIDofTables() {
  ...
  for (i = 0; i < list_length; i++) {
    ...
    for (
      j = 0;
      j < document.querySelectorAll(".task-body")[i].children.length;
      j++
    ) {
      document.querySelectorAll(".task-body")[i].children[j].id =
        "table-body-" + i + "-row-" + j;
    }
  }
  let saved = localStorage.setItem(
...
function addRowToList() {
  ...
  document
    .getElementById(event.target.parentNode.parentNode.parentNode.id)
...
function createRow() {
  ...
  table_checkbox.innerHTML =
    "<input type='checkbox' name='rowcomplete' value='complete' />";
  ...
  row.innerHTML =
    table_checkbox.outerHTML +
    row_data.outerHTML +
    add_row_button.outerHTML +
    delete_row_button.outerHTML;
  row.className = "task-body-row";
  if (
    !event.target.parentNode.parentNode.parentNode.id.includes("table-body")
  ) {
    row.id =
      "table-body-" +
      document.getElementsByClassName("task-list").length +
      "-row-0";
  } else {
    row.id =
      event.target.parentNode.parentNode.parentNode.id +
      "-row-" +
      document.getElementById(event.target.parentNode.parentNode.parentNode.id)
        .rows.length;
  }
  return row;
}
...
window.onload = function () {
  if (localStorage.getItem("task_lists")) {
    ...
    document.querySelector("#task-list-container").innerHTML =
      localStorage.getItem("task_lists");
